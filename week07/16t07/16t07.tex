...
\onslide<2>\textbf{Proof} If all of them are true, it is clear to see that all disjunctions are true so that the conjunction is true. Similarly, if all of them are false, all disjunctions are true so that the conjunction is true. Otherwise, if at least one of them is false and at least one of them is true, without loss of generality, assume $p = False$, $q = True$. Then $p \vee \neg q = False$ so that the conjunction is false.
\end{frame}
