...
\onslide<2>\textbf{Proof} It is easy to evaluate that the expression is true when all variables have the same truth value. Suppose at least one is true and at least one is false. There are 2 combinations of 2 variables which have different truth value. These 2 combinations can form 2 disjunction expression with false value and one of them is in the conjunction expression so that the whole expression is false.
\end{frame}
