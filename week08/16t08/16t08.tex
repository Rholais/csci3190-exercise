...
\begin{frame}[allowframebreaks]
	\frametitle{Circuit}
	\begin{example}
		Let $P_1$ and $P_2$ be two paths without circuit between the vertices $u$ and $v$ in the simple graph $G$ that do not contain the same set of edges. Show that there is a circuit in $G$.
	\end{example}
	\textbf{Proof.}	Let
	the simple paths $P_1$ and $P_2$ be $u = x_0, x_1, \cdots, x_n = v$ and $u = y_0, y_1$, $\cdots$, $y_m = v$, respectively. Since the paths do not contain the same set
	of edges, they must diverge eventually. We can suppose that $x_0 = y_0, x_1 = y_1$, $\cdots$, $x_i = y_i$, but $x_{i + 1} \ne y_{i + 1}$. 
	
	\newpage
	
	To form our circuit, we follow the path $y_i, y_{i + 1}, y_{i + 2}$, and so on,
	...
	to $x_i$. Since $x_i = y_i$, this certainly forms a circuit. It must
	be a simple circuit, since no edge among the $x_k$s or the $y_l$s
	can be repeated and no
	edge among the $x_k$s can equal one of the edges $y_l$ that we used,
	since we abandoned $P_2$ for $P_1$ as soon as we hit $P_1$.
\end{frame}
